...
\begin{multicols}{2} % Begin two column mode
    ...
    \vspace{0.052\textheight} % Vertical whitespace

    \section{Introducción}

    El desarrollo del procesador \textit{jsp} se ha centrado en la experiencia del usuario (\textit{UX}). Para ello, se han priorizado tres aspectos fundamentales una gestión de errores sólida e informativa, una interfaz de línea de comandos (\textit{CLI}) intuitiva y, finalmente, que el programa sea eficiente y rápido.
    ...
    Además, gracias al uso del patrón de \textit{inyección de dependencias} en todo el proyecto, el usuario puede optar por utilizar módulos (como el \textit{Lexer} o la Tabla de Símbolos) que no vuelquen sus contenidos en ficheros, evitando así el \textit{overhead} de las \textit{syscalls}.
